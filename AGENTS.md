...
9. コンテキスト使用率が40%以上になった場合、または40%到達を検知できない環境でも会話・作業履歴が長大化して引き継ぎ損失の危険が高いと判断した場合は、作業を止めずに、対象Project Repository内へ「完全版引き継ぎファイル」を作成または更新してから作業を継続する。詳細は `Context Handoff Protocol` に従う。
...
40%到達は「作業終了」の意味ではありません。**先に完全版引き継ぎを保存し、その後も安全に作業を続行します。**
